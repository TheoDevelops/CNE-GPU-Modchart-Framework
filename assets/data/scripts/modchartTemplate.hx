...
var easeEventsRunning:Array<Dynamic> = [];
...
public function tweenModifierValue(name:String, newValue:Float, time:Float, easeFunc:Float->Float)
{
	...
	/**
	 * Prevents overlapping ease events
	 */
	for (easeEvent in easeEventsRunning)
	{
		if (easeEvent.name == name.toLowerCase()) {
			easeEvent.tween.cancel();
			easeEventsRunning.remove(easeEvent);

			// So, tween cancel functions runs the onComplete function...
			mod[MOD_VALUE] = startValue;
			continue;
		}

		if (Conductor.songPosition >= easeEvent.time + (Conductor.getTimeForStep(easeEvent.length * 4)))
		{
			easeEventsRunning.remove(easeEvent);
			continue;
		}
	...
	var tween = FlxTween.num(startValue, newValue, time,
	{
		onUpdate: (tween) -> {
			// ease properly with lerp
			mod[MOD_VALUE] = FlxMath.lerp(startValue, newValue, easeFunc(tween.percent));
		},
		ease: easeFunc,
		onComplete: (tween) -> {
